...
class SpecialFormatter(logging.Formatter):
    ...
    def format(self, record):
        """Return the correct format for the logger and level.

        If logger is (or is a child of) `.module` logger and is at info level, just print the message.
        Otherwise, return a formatter depending on the log level.
        """
        if ".module" in record.name and record.levelno == logging.INFO:
            log_fmt = "%(message)s"
        else:
...
def init_module_level(logger_name, trace_level=logging.WARNING):
    """Initialize module level logger.

    The logger returned by this function should be used by "modules", which are
    similar to procedures but have no C-code.
    These loggers will all be child to the "*.module" logger, which receives special
    treatment by the top logger's formatter.
    """
    parent_name = get_top_logger().name + ".module"
    # create procedure level logger
    new_logger = logging.getLogger(
        name=parent_name + "." + logger_name,
        )
