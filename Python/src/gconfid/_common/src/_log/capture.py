...
class LogRedirect(ABC):
    ...
    Allows redirection of output (like `print()`, `logging`, C code `printf()`, etc.) to
    a temporary file or existing file, with options to redirect to reprint captured text to
    other files/streams or to be retrieved in a variable.
    ...
    *OPTIONAL*: specify to enable optional function
    ...
    Use as a context manager:
        `.__enter__()`
            ...
            - redirect output
            - returns self, so assignment can occur in the with clause
        `.__exit__()`
            ...
            - store captured text in `captured_text`
                - append captured text to `captured_text_all`

    When a derived class instance is used multiple times, captured text is accumulated in
    `captured_text_all`.
    """
    ...
    def __init__(
        ...
        self.captured_text = ""
        self.all_captured_text = ""
        self.tfile = None
        self.tfile_wrapper = None
        self.tfile_close_on_exit = False
        self.tfile_tell = 0
    ...
    def __enter__(self):
        if self.enabled:
            ...
            # create temporary file, or use `existing_tfile` in append+ binary mode
            if self.existing_tfile is None:
                self._tfile_create()
            else:
                self._tfile_open_existing()

            # call derived class' `start_redirection()`
            self.start_redirection()

        # `return self` allows `with LogRedirect as foo:` type assignment
        return self
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        # get captured text
        self.captured_text = self._tfile_get_captured_text()
        # append to cumulative captured text
        self.all_captured_text += self.captured_text
        self._tfile_release()
    ...
    def print_captured_text(self):
        ...
        if self.reprint_file is None:
            # `end=""`: don't print trailing newline
            print(self.captured_text, end="")  # noqa: T201
        else:
    ...
    def _tfile_create(self):
        ...
        # just created, so it's at the beginning
        self.tfile_tell = 0

        # create tfile wrapper: to write TEXT data (like from loggers or `print()`)
        self.tfile_wrapper = io.TextIOWrapper(
            buffer=self.tfile,
            write_through=True,
            encoding="utf-8",
            newline="",  # `newline=""` disables the newline translation
        )

    def _tfile_open_existing(self):
        ...
        self.tfile_close_on_exit = False  # leave open, managed by caller
    ...
    def _tfile_get_captured_text(self, encoding="utf-8"):
        """Return captured text as a string.

        Reads `self.tfile` from `self.tfile_tell` until end.
        Decodes data into string and returns it.
        """
        ...
        # create buffer for storing captured binary
...
class PythonLogRedirect(LogRedirect):
    """Redirects Python's logger and Python `print()` (stdout and stderr) to a file.

    Redirection of logging is specific to this packages "top logger", as defined in `.._log`
    """
...
class CCodeLogRedirect(LogRedirect):
    """Redirects C code (OS level) stdout and stderr."""
