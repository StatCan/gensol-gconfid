...
def duckdb_load_table(db_conn, tbl_to_load, db_table_name):
    """Load table into duckdb under specified name.

    Loads `tbl_to_load` into a duckdb table named `db_table_name`.

    If pandas 3.0 dtype "str" dtype causes issues, such as with
    duckdb==1.4.3, convert to "string" dtype.
    """
    for _unused in range(2):  # loop: to not repeat the `try` clause
        try:
            db_conn.sql(f"CREATE TABLE {db_table_name} AS SELECT * from tbl_to_load")  # noqa: S608 # no user arguments here, no concerns
            break
        except duckdb.NotImplementedException as e:
            if isinstance(tbl_to_load, pd.DataFrame) and "'str' not recognized" in e.args[0]:
                str_col = tbl_to_load.select_dtypes("str").columns
                tbl_to_load[str_col] = tbl_to_load[str_col].astype("string")
...
def flag_rows_where(ds_in, where_stmt, new_col_name="_exclude", flag_value="E"):
    ...
    try:
        ...
        duckdb_load_table(conn, ds_in, temp_table_name)

        # if column exists (case insensitive)
        if conn.sql(f"""
            SELECT * FROM information_schema.columns
            WHERE table_name='{temp_table_name}' AND LOWER(column_name)=LOWER('{new_col_name}')
        """).to_arrow_table().num_rows > 0:  # noqa: S608 # private temporary database, no concerns
            # add a random suffix
        ...
        # modify
        conn.sql(f"ALTER TABLE {temp_table_name} ADD COLUMN {new_col_name} VARCHAR")  # noqa: S608 # private temporary database, no concerns
        conn.sql(f"UPDATE {temp_table_name} SET {new_col_name} = '{flag_value}' WHERE {where_stmt}")  # noqa: S608 # private temporary database, no concerns

        # export from duckdb
        if isinstance(ds_in, pd.DataFrame):
            ds_out = conn.sql(f"SELECT * from {temp_table_name}").df()  # noqa: S608 # private temporary database, no concerns
            ds_out = handle_pandas_string_data(ds_out)
        elif isinstance(ds_in, pa.Table):
            ds_out = conn.sql(f"SELECT * from {temp_table_name}").to_arrow_table()  # noqa: S608 # private temporary database, no concerns
    finally:
        conn.close()
...
def remove_rows_where(ds_in, where_stmt):
    ...
    try:
        ...
        # modify
        conn.sql(f"DELETE FROM {temp_table_name} WHERE {where_stmt}")  # noqa: S608 # private temporary database, no concerns
