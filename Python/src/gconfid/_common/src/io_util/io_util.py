...
# create list of tuples, one per supported file type
SUPPORTED_INPUT_FORMATS = [   # ("format-name", "loaded format", <file extensions>, <optional warning message>)
    ("Arrow IPC", "pyarrow.Table", tc.file_ext_arrowipc, tc.PAT_from_arrowipc_file, None),
    ("Feather", "pyarrow.Table", tc.file_ext_feather, tc.PAT_from_feather_file, None),
    ("Parquet", "pyarrow.Table", tc.file_ext_parquet, tc.PAT_from_parquet_file, None),
    ("CSV", "pyarrow.Table", tc.file_ext_csv, tc.PAT_from_csv_file,
        _("csv files are not recommended for input due to low numeric precision and inconsistent data types")),
    ("SAS Dataset", "pandas.DataFrame", tc.file_ext_sas, tc.DF_from_sas_file, None),
]

# create list of tuples, one per supported file type
#  at least one "writer func" must be defined, the other *can* be `None`
SUPPORTED_OUTPUT_FORMATS = [  # ("format name", <file extensions>, <pyarrow writer func>, <pandas writer func>)
    ("Arrow IPC", tc.file_ext_arrowipc, tc.PAT_to_arrowipc_file, None),
    ("CSV", tc.file_ext_csv, tc.PAT_to_csv_file, tc.DF_to_csv_file),
    ("Feather", tc.file_ext_feather, tc.PAT_to_feather_file, tc.DF_to_feather_file),
    ("Parquet", tc.file_ext_parquet, tc.PAT_to_parquet_file, tc.DF_to_parquet_file),
]

class StcTable(ABC):
    """Abstract Base Class for table wrapper classes.

    All input and output tables share the following attributes

    - `name`: string naming the table
    - `user_spec`: for storeing what the user originally specified
    - `mandatory`: boolean identifying if dataset is mandatory or not
    - `ds_intermediate`: holds the dataset when loaded into an "intermediate" format

    The following properties inspect the user specified value

    - `is_specified`: boolean value indicating if `user_spec` value indicates a user having specified a table
    - `not_specified`: the opposite of `is_specified`

    The remaining properties and method require the `ds_intermediate` attribute to be populated with
    a table in either of the the `pandas.DataFrame` or `pyarrow.Table` format.

    - `num_rows`: number of rows in table
    - `num_columns`: number of columns in the table
    - `get_ds_info()`: returns a string describing various metadata about the table
    - `log_metadata()`: given a logger, log select metadata at the info and debug levels
    - `to_arrow()`: get table in `pyarrow.Table` format
    - `to_pandas()`: get table in `pandas.DataFrame` format


    Subclasses must

    - store values in attributes as needed
    - and then call the superclass constructor
    - implement the `validate_user_spec()` method
    """
    ...
    def is_specified(self):
        """Whether or not a table is considered specified by the user."""
        return self.user_spec is not None

    @property
    def not_specified(self):
        """Whether or not a table is considered "not specified"."""
...
class StcInputTable(StcTable):
    """Wrapper for input tables.

    This class extends on the `StcTable` class only by implementing a validation method
    that ensures mandatory datasets are specified.
    """
    ...
    def validate_user_spec(self):
        """Validate the user specification."""
        # disallow specifying None for mandatory input datasets
        if self.mandatory is True and self.not_specified:
            mesg = _("Mandatory dataset '{}' must be specified").format(self.name)
            raise ValueError(mesg)

class StcOutputTable(StcTable):
    """Wrapper for output tables.

    This class extends on the `StcTable` class by implementing the `validate_user_spec()` method
    and adding the following attributes

    - `user_output`: a place to store output datasets in the formated requested in `user_spec`
    - `requested_by_default`: whether a dataset should be generated even when it is not mandatory and is `not_specified`
    """
    ...
    def is_requested(self):
        ...
        Otherwise, this is determined by the value of `user_spec`
          - `None` means *use default* (default is stored in `requested_by_default`)
...
class GensysInputDataset(StcInputTable):
    ...
    def validate_user_spec(self):
        """Validate the user specification."""
        # currently handled by C code, ignore superclass implementations

class GensysOutputDataset(StcOutputTable):
    ...
    def __init__(self, name, output_specification, mandatory=True, requested_by_default=True):
        ...
        super().__init__(
            ...
            mandatory=mandatory,
            requested_by_default=requested_by_default,
        )
...
def interm_from_input_file(dataset_ref, log):
    ...
    for fmt_name, dest_fmt, file_exts, load_func, warn_text in SUPPORTED_INPUT_FORMATS:
        if fn_ext in file_exts:
            # CONTEXT: Loading <file format> file into <object format>
            log_lcl.debug(_("Loading {} file into `{}`").format(fmt_name, dest_fmt))
            if warn_text is not None:
                log_lcl.warning(warn_text)
            return load_func(dataset_ref)
...
def interm_to_output_file(dataset, destination, log):
    ...
    if not isinstance(dataset, (pd.DataFrame, pa.Table)):
        mesg = _("Invalid type: `{}`, supports types: {}").format(str(type(dataset)), "`pandas.DataFrame`, `pyarrow.Table`")
        log_lcl.error(mesg)
        raise TypeError(mesg)

    # get file extension and call the appropriate writer function
    fn_ext = Path(destination).suffix
    for fmt_name, file_extensions, pat_func, df_func in SUPPORTED_OUTPUT_FORMATS:
        if fn_ext in file_extensions:
            # CONTEXT: Saving dataset to <file format> file: '<file path>'
            log_lcl.debug(_("Saving dataset to {} file: '{}'").format(fmt_name, destination))
            if isinstance(dataset, pa.Table):
                if pat_func is None:
                    df_func(tc.PAT_to_DF(dataset), destination)
                else:
                    pat_func(dataset, destination)
                return
            if isinstance(dataset, pd.DataFrame):
                if df_func is None:
                    pat_func(tc.DF_to_PAT(dataset), destination)
                else:
                    df_func(dataset, destination)
                return
...
def write_output_dataset(dataset, destination, log, default_format=None):
    ...
    if destination is None or destination is True:
        dest = default_format if default_format is not None else tc.get_default_output_spec()
        # default action: `None`- implicitly requested default type, `True`-explicitly requested default type
        log_lcl.debug(_("No destination specified, returning default format: '{}'").format(dest))
    ...
    if dest_is_file(destination=destination):
        ...
        interm_to_output_file(dataset, str(dst), log_lcl)
        return str(dst)
