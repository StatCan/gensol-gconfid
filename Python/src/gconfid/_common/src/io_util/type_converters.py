...
OPT_feather_mmap_mode_default = feather_mmap_mode.pa_feather_mmap
...
OPT_arrowipc_use_mmap_default=True
OPT_arrowipc_use_mmap=OPT_arrowipc_use_mmap_default
...
file_ext_arrowipc = [".arrow"]  # does NOT include `.arrows` which is for arrow ipc STREAMING format, not FILE format
...
def set_default_output_spec(new_spec):
    ...
    validate_output_spec(new_spec)

    global default_output_specification
    default_output_specification = new_spec

def validate_output_spec(spec):
    """Validate that output specification is supported."""
    if spec not in supported_output_obj_id:
        mesg = _("Specification '{}' not supported, must use one of '{}'").format(spec, supported_output_obj_id)
        raise ValueError(mesg)

# Disable/Enable the split_blocks option when converting a PAT to Pandas DF
OPT_pandas_use_split_blocks_default=True
OPT_pandas_use_split_blocks=OPT_pandas_use_split_blocks_default

def get_OPT_split_blocks() -> bool:
    """Return current split_blocks option setting."""
    return OPT_pandas_use_split_blocks

def set_OPT_split_blocks(new_opt: bool) -> None:
    """Set OPT_pandas_use_split_blocks to a supported value."""
    if new_opt not in (True, False):
        mesg = _("Option OPT_pandas_use_split_blocks only supports boolean values, '{}' is not supported.").format(new_opt)
        raise ValueError(mesg)

    global OPT_pandas_use_split_blocks
    OPT_pandas_use_split_blocks = new_opt
...
def PAT_from_arrowipc_file(
        file_path,
        memory_map : bool | None = None,
):
    """Load Arrow IPC file into PyArrow Table (PAT).

    `memory_map`:   Whether or not to use memory mapping.What memory map option to use.
                    Can be `True`, `False`, or a `none` (unspecified) to get default behaviour.
    """
    if isinstance(file_path, Path):
        file_path = str(file_path)
    if memory_map is None:  # if True, use default
        memory_map = OPT_arrowipc_use_mmap
    elif not isinstance(memory_map, bool):
        raise TypeError
    ...
    try:
        ...
        mesg = _("Unexpected error occurred while loading {} file: '{}'").format("Arrow IPC", file_path)
        raise TypeConverterError(mesg) from e
    else:
        return pa_t
...
def PAT_from_feather_file(
    ...
    """
    if memory_map is True:  # if True, use feather reader
        memory_map = feather_mmap_mode.pa_feather_mmap
    ...
    try:
        match memory_map:
            case feather_mmap_mode.pa_ipc_mmap:
                pa_t = PAT_from_arrowipc_file(file_path, memory_map=True)
...
def PAT_to_DF(pa_t):
    ...
    """
    split_blocks = OPT_pandas_use_split_blocks
    if split_blocks not in (True, False):  # if global option invalid, use default
        split_blocks = OPT_pandas_use_split_blocks_default
...
def PAT_to_arrowipc_file(dataset, destination):
    """Write PyArrow Table to arrow IPC file."""
    try:
        with pa.ipc.new_file(str(destination), dataset.schema) as writer:
            writer.write_table(dataset)
    except Exception as e:
        mesg = _("Unexpected error occurred while writing `{}` to {} file: '{}'").format("pyarrow.Table", "Arrow IPC", destination)
        raise TypeConverterError(mesg) from e
...
def SAS_file_to_arrowipc_file(file_path, destination):
    """Read SAS file and write to Arrow IPC file."""
    pa_t = DF_to_PAT(
        DF_from_sas_file(file_path),
    )
    PAT_to_arrowipc_file(dataset=pa_t, destination=destination)
