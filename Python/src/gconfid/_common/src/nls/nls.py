...
def _match_language_string(lang_str: str | None) -> SupportedLanguage:
    """Normalize language string and match it against SupportedLanguage."""
    if not lang_str:
        return SupportedLanguage.UNKNOWN

    # Normalize 'en_US' -> 'en', 'English_United States' -> 'english'
    normalized = lang_str.lower().split("_")[0]

    for sl in SupportedLanguage:
        if normalized in sl.value:
            return sl
...
def determine_language() -> SupportedLanguage:
    ...
    Extract language from `locale.getlocale()`, or if that fails, the "LANG" environment variable.
    Match against supported language.
    ...
    try:
        ...
        lang_match = _match_language_string(current_locale)

        if lang_match != SupportedLanguage.UNKNOWN:
            return lang_match

    except (IndexError, ValueError):
        pass  # really, just try the next method

    lang_env = os.environ.get("LANG")
    lang_match = _match_language_string(lang_env)
