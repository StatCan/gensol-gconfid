...
class GeneralizedProcedure(ABC):
    ...
    _default_args = {
        ...
        "trace" : False,
        "default_output_format" : None, # Set this to override the type_converter default when writing outputs for this proc only
    }
    ...
    def captured_text(self):
        """Console output text captured during procedure execution.

        This attribute will be populated in cases where capturing is enabled,
        such as when `capture=True` is specified.
        """
    ...
    def _init_capture(self, capture, logger, keyword_args, log):  # noqa: ARG002  # `keyword_args` reserved for future/subclass use
        ...
        Configures the `self._CCodeLogRedirect` and `self._py_log_redirect` context managers to handle
        ...
        if isinstance(logger, logging.Logger):
            ...
            self._CCodeLogRedirect = lg.capture.CCodeLogRedirect(enabled=True, reprint=True, reprint_file=logger)
            ...
            self._CCodeLogRedirect = lg.capture.CCodeLogRedirect(enabled=True, reprint=False)
            self._py_log_redirect = lg.capture.PythonLogRedirect(enabled=True, reprint=False)
            ...
            self._CCodeLogRedirect = lg.capture.CCodeLogRedirect(enabled=True, reprint=True)
            ...
            self._CCodeLogRedirect = lg.capture.CCodeLogRedirect(enabled=False)
            self._py_log_redirect = lg.capture.PythonLogRedirect(enabled=False)
        else:
    ...
    def _run_proc(self, log=None):
        ...
        Logging
            ...
            `_CCodeLogRedirect` redirects C `stdout` and `stderr` output as configured during initialization
            ...
            if ds.is_requested:
                ...
                with diag.SystemStats(_("output '{}'").format(ds.name), logger=log_lcl):
                    try:
                        ...
                        ds.user_output = io.write_output_dataset(ds.ds_intermediate, ds.user_spec,
                                                                 log_lcl, default_format=self.get_default_output_format())
                    except Exception as e:
    ...
    def get_default_output_format(self) -> str:
        ...
        return self.get_default("default_output_format")

    @classmethod
    def set_default_output_format(cls, value: str) -> None:
        """Validate and set the default output format."""
        from gconfid._common.src.io_util.type_converters import validate_output_spec
        validate_output_spec(value)
        cls.set_default("default_output_format", value)
