...
)
{
	...
	my_bcopy ((char *) & argv[last_nonopt], (char *) & argv[first_nonopt], (optind - last_nonopt) * sizeof (char *));
	my_bcopy ((char *) temp, (char *) & argv[first_nonopt + optind - last_nonopt], nonopts_size);
...
)
{
	...
	if (optind == 0) {
		...
		if (optstring[0] == '-') {
			...
			++optstring;
		} else if (getenv ("POSIXLY_CORRECT") != NULL)  // nosemgrep  // value isn't used, its safe
			ordering = REQUIRE_ORDER;
			else
			ordering = PERMUTE;
	}
...
)
{
	...
	while (1) {
		...
		case '?':
			printf ("USAGE: getopt abc:d:0123456789\n");
			break;
